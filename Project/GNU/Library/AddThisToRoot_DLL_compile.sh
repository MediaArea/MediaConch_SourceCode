...
if [ "$OS" = "mac" ] ; then
    export CXXFLAGS="-mmacosx-version-min=10.9 -arch x86_64 -arch arm64 $CXXFLAGS"
    export CFLAGS="-mmacosx-version-min=10.9 -arch x86_64 -arch arm64 $CFLAGS"
    export LDFLAGS="-mmacosx-version-min=10.9 -arch x86_64 -arch arm64 $LDFLAGS"
fi
...
if test -e MediaInfoLib/Project/GNU/Library/configure; then
    ...
    if [ "$OS" = "mac" ]; then
        ...
        ./configure --enable-staticlibs --enable-static --disable-shared --with-libcurl=runtime $*
    fi
    if test ! -e Makefile; then
...
if test -e MediaConch/Project/GNU/Library/configure; then
    cd MediaConch/Project/GNU/Library/
    test -e Makefile && rm Makefile
    chmod +x configure
    if [ "$OS" = "mac" ]; then
        ./configure $MacOptions --enable-shared --enable-staticlibs $*
    else
        ./configure --enable-shared --enable-staticlibs $*
    fi
    if test ! -e Makefile; then
        echo "Problem while configuring MediaConch (Library)"
        exit
    fi
else
...
make clean
Parallel_Make
if test -e .libs/libmediaconch.la ; then
    echo "MediaConch (Library) compiled"
else
    echo "Problem while compiling MediaConch (Library)"
    exit
fi
cd $Home
...
echo "MediaConch shared object is in MediaConch/Project/GNU/Library/.libs"
echo "For installing, cd MediaConch/Project/GNU/Library && make install"
